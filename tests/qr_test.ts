...
Deno.test("qr - create - should return a valid request data object", () => {
  ...
  const body: MerchantCallbackRequest = { locationDescription: "your-location-description" };
...
Deno.test("qr - info - should return a valid request data object", () => {
  ...
  const requestData = callbackQRRequestFactory.info(token, merchantQrId, qrImageFormat, qrImageSize);

  assert(requestData.url, "/qr/v1/merchant-callback/your-merchant-qr-id?QrImageFormat=SVG&QrImageSize=200");
...
Deno.test("qr - list - should return a valid request data object", () => {
  ...
  const requestData = callbackQRRequestFactory.list(token, qrImageFormat, qrImageSize);

  assert(requestData.url, "/qr/v1/merchant-callback?QrImageFormat=SVG&QrImageSize=200");
  assert(requestData.method, "GET");
  assert(requestData.token, token);
});
...
Deno.test("qr - createMobilePayQR - should return a valid request data object", () => {
  ...
  const requestData = callbackQRRequestFactory.createMobilePayQR(token, beaconId, body);
